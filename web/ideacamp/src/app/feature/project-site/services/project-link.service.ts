...
export class ProjectLinkService {
  ...
  getProjectLinks(projectId: string): Observable<ProjectLinkModel[]> {
    return this.http
      .get<unknown>(`${this.baseUrl}/${projectId}/links`)
      .pipe(map((data) => z.array(ProjectLinkSchema).parse(data)));
  }

  addProjectLink(projectId: string, request: CreateLinkRequest): Observable<ProjectLinkModel> {
    return this.http
      .post<unknown>(`${this.baseUrl}/${projectId}/links`, request)
      .pipe(map((data) => ProjectLinkSchema.parse(data)));
  }

  updateProjectLink(projectId: string, linkId: string, request: UpdateLinkRequest): Observable<ProjectLinkModel> {
    return this.http
      .patch<unknown>(`${this.baseUrl}/${projectId}/links/${linkId}`, request)
      .pipe(map((data) => ProjectLinkSchema.parse(data)));
  }
